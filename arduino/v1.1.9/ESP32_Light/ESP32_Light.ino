...
//
// Her ürün için aşağıdaki 4 tanımı doldur ve flaşla.
// İlk açılışta Preferences'a kaydedilir, OTA'da korunur.
//
// PARTS: Virgülle ayrılmış part key listesi (sıra önemli — alttan üste)
#define DEVICE_PARTS_DEFAULT "base01,body02,head01"

// COLORS: Her part için hex renk kodu
#define DEVICE_COLORS_DEFAULT "{\"base01\":\"#1a1a1a\",\"body02\":\"#c8b89a\",\"head01\":\"#2a2a2a\"}"

// ROUGHNESS: 0.0 (parlak) → 1.0 (mat)
#define DEVICE_ROUGHNESS_DEFAULT "{\"base01\":0.8,\"body02\":0.6,\"head01\":0.9}"

// METALNESS: 0.0 (plastik) → 1.0 (metal)
...
// ── PARTS VERSİYONU ───────────────────────────────────────────────────────────
// Parts listesini değiştirince bu sayıyı 1 artır.
// Preferences'taki eski versiyon farklıysa yeni değerler yazılır.
// OTA güncellemesinde parts değişmesini istemiyorsan bu sayıyı değiştirme.
#define DEVICE_PARTS_VERSION 3  // ← her parts değişikliğinde artır
...
void loadOrInitParts() {
  ...
  if (savedVersion != DEVICE_PARTS_VERSION) {
    // İlk çalıştırma VEYA versiyon değişmiş — yeni değerleri kaydet
    ...
    deviceColors    = DEVICE_COLORS_DEFAULT;
    deviceRoughness = DEVICE_ROUGHNESS_DEFAULT;
    deviceMetalness = DEVICE_METALNESS_DEFAULT;
    Serial.printf("✅ Parts güncellendi (v%d): %s\n", DEVICE_PARTS_VERSION, deviceParts.c_str());
  } else {
    // Güncel — Preferences'tan oku

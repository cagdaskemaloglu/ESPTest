...
#define VERSION_JSON_URL "https://raw.githubusercontent.com/cagdaskemaloglu/torva-firmware/main/firmware/ESP32_Light.ino.bin"

// ── PARTS TANIMI — ÜRETİMDE BİR KEZ AYARLA ───────────────────────────────────
//
// Her ürün modeli için bu listeyi değiştir ve flaşla.
// Bu değer ilk çalıştırmada Preferences'a kaydedilir.
// OTA güncellemelerinde Preferences silinmez — parts korunur.
//
// Format: virgülle ayrılmış key listesi, boşluk olmamalı
// Örnekler:
//   "body,strip_0"                    → tek şeritli, 2 parça
//   "body,strip_0,strip_1"            → çift şeritli, 3 parça
//   "cylinder_body,top_ring,strip_0"  → özel model
//
// Kaç key girersen o model o kadar parçaya sahip demektir (max 10)
//
#define DEVICE_PARTS_DEFAULT "body,strip_0"

// Kaç kanal LED şeridi kontrol ediliyor?
// 1 = tek şerit (ControlScreen tek kanal gösterir)
// 2 = çift şerit (ControlScreen iki kanal gösterir)
#define NUM_CHANNELS 1

// Runtime'da Preferences'tan okunur
String deviceParts = "";
...
bool applyUpdate(const String& binUrl) {
  ...
  // İlerleme callback — sadece Serial log
...
void factoryReset() {
  ...
  // Parts'ı silme — fabrika sıfırlamasında parts korunur
  // Sadece WiFi, PIN ve automation kuralları sıfırlanır
  delay(500);
  ESP.restart();
}
...
// ── Parts yükle / ilk kez kaydet ─────────────────────────────────────────────
// İlk çalıştırmada DEVICE_PARTS_DEFAULT Preferences'a kaydedilir.
// Sonraki çalıştırmalarda (OTA dahil) Preferences'tan okunur.
// Böylece her üründe bir kez flaşlamak yeterlidir.
void loadOrInitParts() {
  prefs.begin("device", false);
  String saved = prefs.getString("parts", "");

  if (saved == "") {
    // İlk çalıştırma — varsayılanı kaydet
    prefs.putString("parts", DEVICE_PARTS_DEFAULT);
    deviceParts = DEVICE_PARTS_DEFAULT;
    Serial.println("✅ Parts ilk kez kaydedildi: " + deviceParts);
  } else {
    // Daha önce kaydedilmiş — oku
    deviceParts = saved;
    Serial.println("📦 Parts yüklendi: " + deviceParts);
  }

  prefs.end();
}

// Parts listesini JSON array string'e çevir
// "body,strip_0,strip_1" → ["body","strip_0","strip_1"]
String partsToJsonArray(const String& parts) {
  ...
  for (int i = 0; i <= (int)parts.length(); i++) {
    if (i == (int)parts.length() || parts[i] == ',') {
      String key = parts.substring(start, i);
      key.trim();
      if (key.length() > 0) {
        if (!first) json += ",";
        json += "\"" + key + "\"";
...
void handleWhoAmI() {
  ...
  // Parts listesini Preferences'tan oku
  String partsJson = partsToJsonArray(deviceParts);

  // Channels — tek kanallı
  String channelsJson =
    "[{\"id\":0,\"name\":\"Şerit\","
...
void setup() {
  ...
  // Parts — ilk çalıştırmada kaydet, sonrakilerde oku
  loadOrInitParts();

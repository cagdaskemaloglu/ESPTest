...
#include <esp_mac.h>  // esp_read_mac için
...
String password  = "";
String devicePin   = "";
String deviceSerial = ""; // İlk açılışta üretilen benzersiz kimlik — Preferences'ta kalıcı
...
// İlk açılışta ESP32 MAC adresinden benzersiz bir seri no üretilir.
// Preferences'ta "device" namespace'inde "serial" key'i ile saklanır.
// OTA ve fabrika sıfırlamasında KORUNUR — sadece flash'layınca sıfırlanır.
void loadOrInitSerial() {
  prefs.begin("device", false);
  deviceSerial = prefs.getString("serial", "");
  if (deviceSerial == "") {
    // MAC adresi + millis kombinasyonu ile benzersiz ID üret
    uint8_t mac[6];
    esp_read_mac(mac, ESP_MAC_WIFI_STA);
    char buf[32];
    snprintf(buf, sizeof(buf), "AB-%02X%02X%02X%02X%02X%02X",
             mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
    deviceSerial = String(buf);
    prefs.putString("serial", deviceSerial);
    Serial.printf("✅ Seri no üretildi: %s\n", deviceSerial.c_str());
  } else {
    Serial.printf("📋 Seri no: %s\n", deviceSerial.c_str());
  }
  prefs.end();
}
...
void runFade() {
  ...
  if (elapsed >= fadeDurationMs) {
    fadeActive = false;
    if (fadeOffAtEnd) {
      // Işığı kapat ama parlaklığı fade öncesi değerine geri yükle
      // — sonraki açılışta ışık görünür olsun
      currentBrightness = fadeStartBri;
      activeEffect = "off"; ledIsOn = false;
      for (int i = 0; i < NUM_LEDS; i++) leds[i] = CRGB::Black;
      FastLED.show();
      Serial.printf("🌙 Fade tamamlandı — ışık kapatıldı, parlaklık %d olarak korundu\n", currentBrightness);
    } else {
